<template>
  <div class="relative">
    ...
    <transition name="modalBgTransition">
      <div v-if="isModalOpen && !isLoggedIn"
    ...
    <button :class="[isDropdownOpen ? '-rotate-90' : 'rotate-0']"
    ...
    <transition name="dropdownTransition">
      <div v-if="isDropdownOpen && !hideModals" v-on-clickaway="closeDropdown"
        ...
        <div
          ...
          <nuxt-link :to="localePath('index')" v-slot="{ navigate, href, isExactActive }">
            <a @click="navigate" :href="href" @keydown.esc.exact="isDropdownOpen = false"
          ...
          <nuxt-link :to="localePath('bountyplatform')" v-slot="{ navigate, href, isActive }">
            <a @keydown.esc.exact="isDropdownOpen = false" @click="navigate" :href="href"
              class="hover:bg-c-background-15 focus:bg-c-background-15 w-full flex flex-row py-2 my-1 justify-center items-center transition-all ease-out duration-200 rounded-lg px-4">
              <div class="flex flex-col items-center">
          ...
          >{{ $t("navigation.dex") }}</a>-->
          <!-- Theme Switch -->
          <button @keydown.esc.exact="isDropdownOpen = false" @keydown.tab.exact="isLoggedIn ? isDropdownOpen = false : null"
            @click="changeTheme"
            ...
            <Icon class="w-6 h-6 mr-1" colorClass="text-c-background"
              :type="$store.state.theme == 'dark' ? 'light' : 'dark'" />
            <div class="text-xl font-bold">{{ $store.state.theme == 'dark' ? $t("theme.light") : $t("theme.dark") }}</div>
          </button>
          <!-- Sign In Button -->
          <button @keydown.esc.exact="isDropdownOpen = false" @keydown.tab.exact="isDropdownOpen = false" v-if="!isLoggedIn"
