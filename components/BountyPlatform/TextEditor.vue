<template>
  <div :class="type == 'feedback' ? 'bg-c-text-05' : 'bg-c-background-ter'" class="w-full relative mt-3 rounded-lg">
    <client-only>
      <div class="bg-c-background-ter rounded-tl-lg rounded-tr-lg">
        <editor-menu-bar v-if="type != 'feedback'" class="bg-c-text-10 rounded-tl-lg rounded-tr-lg px-2" :editor="editor"
          v-slot="{ commands, isActive, getMarkAttrs }">
          <div class="w-full flex flex-row flex-wrap items-center">
            <button :class="{ 'bg-c-text-15': isActive.bold() }"
      ...
      <editor-menu-bubble class="menububble shadow-xl transform"
        :class="linkMenuIsActive ? '-translate-x-1/2' : '-translate-x-1/2'" :editor="editor" @hide="hideLinkMenu"
        ...
        <div class="menububble" :class="{ 'is-active': menu.isActive }"
          ...
          <template v-else>
            <!-- Bold -->
            <button :class="{ 'bg-c-light-25': isActive.bold() }"
            ...
            <!-- Italic -->
            <button :class="{ 'bg-c-light-25': isActive.italic() }"
              class="p-0_5 mx-0_5 rounded-lg hover:bg-c-light-15 transition-colors duration-200 ease-out"
              @click="commands.italic()">
              <icon class="w-6 h-6" type="italic" colorClass="text-c-light" />
            </button>
            <!-- Heading 2 -->
            <button v-if="type != 'feedback'" :class="{ 'bg-c-light-25': isActive.heading({ level: 2 }) }"
              class="p-0_5 mx-0_5 rounded-lg hover:bg-c-light-15 transition-colors duration-200 ease-out"
              @click="commands.heading({ level: 2 })">
              <icon class="w-6 h-6" type="h2" colorClass="text-c-light" />
            </button>
            <!-- Link -->
            <button :class="{ 'bg-c-light-25': isActive.link() }"
              class="p-0_5 mx-0_5 rounded-lg hover:bg-c-light-15 transition-colors duration-200 ease-out"
              @click="showLinkMenu(getMarkAttrs('link'))">
              <icon class="w-6 h-6" type="link" colorClass="text-c-light" />
            </button>
            <!-- Code -->
            <button v-if="type != 'feedback'" :class="{ 'bg-c-light-25': isActive.code() }"
              class="p-0_5 mx-0_5 rounded-lg hover:bg-c-light-15 transition-colors duration-200 ease-out"
              @click="commands.code()">
