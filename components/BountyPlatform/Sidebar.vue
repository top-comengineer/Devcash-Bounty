<template>
  <div
    ...
    @mouseenter="showSidebarTextOnMd()" @mouseleave="hideSidebarTextOnMd()">
    <!-- Sidebar Content -->
    <div class="w-full flex flex-row md:flex-col justify-center py-0 md:py-8">
      <!-- Post -->
      <nuxt-link :to="localePath('bountyplatform-post')"
        ...
        <h3
          ...
          class="text-lg font-bold hidden ml-2">{{ $t("bountyPlatform.post.header") }}</h3>
      </nuxt-link>
      <!-- Explore -->
      <nuxt-link :to="localePath('bountyplatform')"
        ...
        <h3
          ...
          class="text-lg font-bold hidden ml-2">{{ $t("bountyPlatform.explore.header") }}</h3>
      </nuxt-link>
      <!-- Overview -->
      <nuxt-link :to="localePath('bountyplatform-overview')"
        class="flex flex-row justify-center md:justify-start items-center px-4 py-3 md:px-6 transition-colors ease-out duration-200"
        :class="[{
        ...
        <h3
          ...
          class="text-lg font-bold hidden ml-2">{{ $t("bountyPlatform.overview.header") }}</h3>

      </nuxt-link>
      <!-- Bounty Manager -->
      <nuxt-link :to="localePath('bountyplatform-bountymanager')"
        class="flex flex-row justify-center md:justify-start items-center px-4 py-3 md:px-6 transition-colors ease-out duration-200"
        :class="[{
        ...
        }]" v-show="isBountyManager" v-if="!bountiesLoading">
        <div class="w-7 h-7" bountyManagerTab>
          <Icon type="bounty-manager"
        ...
        <h3
          ...
          class="text-lg font-bold hidden ml-2">{{ $t("bountyPlatform.bountyManager.header") }}</h3>
      </nuxt-link>
      <!-- Bounty Hunter-->
      <nuxt-link :to="localePath('bountyplatform-bountyhunter')"
        class="flex flex-row justify-center md:justify-start items-center px-4 py-3 md:px-6 transition-colors ease-out duration-200"
        :class="[{
        ...
        }]" v-show="isBountyHunter" v-if="!bountiesLoading">
        <div class="w-7 h-7" id="bountyHunterTab">
          <Icon type="bounty-hunter"
        ...
        <h3
          ...
          class="text-lg font-bold hidden ml-2">{{ $t("bountyPlatform.bountyHunter.header") }}</h3>
      </nuxt-link>
      <!-- Divider -->
      <div :class="isSidebarTextVisibleOnMd ? 'md:block' : 'lg:block'" class="px-4 md:px-6 mt-4 mb-5 hidden">
      ...
      <div :class="isSidebarTextVisibleOnMd ? 'md:block' : 'lg:block'" class="px-4 md:px-6 hidden">
        ...
        <div v-if="$store.state.sidebarContext == sidebarContexts.explore" class="flex flex-col">
          ...
          <div class="flex flex-row relative mt-2">
            ...
            <input id="searchBar" v-model="searchText" @focus="isSearchFocused = true" @blur="isSearchFocused = false"
              ...
              type="text" :placeholder="$t('bountyPlatform.explore.sidebar.searchPlaceholder')" />
          </div>
          <!-- Sort Text and Sort Options -->
          <div class="relative">
            <!-- Sort Button -->
            <button @click="toggleSortModal" @keydown.esc.exact="hideSortModal"
              ...
              <Icon class="w-4 h-4 mx-1 transition-all ease-out duration-200" colorClass="text-c-text"
                type="arrow-down" />
            </button>
            <!-- Sort Modal -->
            <transition name="modalScaleTransition">
              <!-- Modal Wrapper -->
              <div v-on-clickaway="hideSortModal" class="origin-top-left absolute left-0 pt-2 z-50"
                ...
                <div
                  ...
                  <!-- Descending Button -->
                  <button @click="orderChanged('desc'); hideSortModalWithDelay()" @keydown.esc.exact="hideSortModal"
                  ...
                  <button @click="orderChanged('asc'); hideSortModalWithDelay()" @keydown.tab.exact="hideSortModal"
                    ...
                    class="flex flex-row items-center py-2 transition-colors duration-200 ease-out">
                    <div class="ml-3 mr-2">
                      <Icon :colorClass="sortDirection == 'asc' ? 'text-c-light' : 'text-transparent'" type="done"
                        class="w-5 h-5" />
                    </div>
                    <h6 class="whitespace-no-wrap font-bold">{{ $t("bountyPlatform.explore.sidebar.sortAscending") }}</h6>
                  </button>
                </div>
              </div>
            </transition>
          </div>
          <RadioButton :checked="this.sortType == 'recency'" class="mt-3 -ml-1" name="recency"
            :text="$t('bountyPlatform.explore.sidebar.sortRecency')" :changeCallback="sortChanged" />
          <RadioButton :checked="this.sortType == 'value'" class="mt-3 -ml-1" name="value"
            :text="$t('bountyPlatform.explore.sidebar.sortValue')" :changeCallback="sortChanged" />
          <RadioButton :checked="this.sortType == 'expiry'" class="mt-3 -ml-1" name="expiry"
            :text="$t('bountyPlatform.explore.sidebar.sortExpiry')" :changeCallback="sortChanged" />
          <!-- Status Text and Status Options -->
          <h4 class="text-c-text text-lg font-bold mt-8">{{ $t("bountyPlatform.explore.sidebar.statusHeader") }}</h4>
          <CheckmarkButton :checked="status.active" :callback="activeCheckChanged" class="mt-3 -ml-1"
            :text="$t('bountyPlatform.explore.sidebar.statusActive')" />
          <CheckmarkButton :checked="status.completed" :callback="completedCheckChanged" class="mt-3 -ml-1"
            :text="$t('bountyPlatform.explore.sidebar.statusCompleted')" />
          <CheckmarkButton :checked="status.expired" :callback="expiredCheckChanged" class="mt-3 -ml-1"
        ...
        <div
          ...
          class="flex flex-col">
          <!-- Balance Text -->
          <h5 v-if="!$store.state.devcashData.ethIsPrimary" class="text-c-text text-sm opacity-75">
            {{ $t('bountyPlatform.sidebarContextual.balance') }}</h5>
          <h4 v-if="!$store.state.devcashData.ethIsPrimary"
          ...
          <!-- Balance Approved -->
          <h5 class="text-c-text text-sm opacity-75">{{ $t('bountyPlatform.sidebarContextual.approvedBalance') }}</h5>
          <h4 class="font-extrabold text-c-primary text-xl mt-1 break-all">{{
            `${balance.primary.symbol}${balance.primary.approved ? balance.primary.approved : balance.primary.amount}` }}
          </h4>
          ...
          <div v-if="balance.primary.hasApproved" class="flex flex-row items-center relative mt-2">
            ...
            <input v-model="toApprove" name="amountToApprove"
              ...
              type="number" min="1" :placeholder="$t('bountyPlatform.sidebarContextual.amountToApprovePlaceholder')" />
          </div>
          <!-- Approve Button -->
        ...
        <div v-else-if="$store.state.sidebarContext == sidebarContexts.singleBounty" class="flex flex-col">
          <nuxt-link :to="localePath('bountyplatform')"
    ...
    <transition name="modalBgTransition">
      <div v-if="isApprovalLoadingModalVisible"
        ...
        <div class="max-w-xl h-full flex flex-row justify-center items-center px-2 pt-24 pb-12 md:pt-36">
          <multi-purpose-modal :header="$t('bountyPlatform.multiPurposeModal.approveAmount.header')"
