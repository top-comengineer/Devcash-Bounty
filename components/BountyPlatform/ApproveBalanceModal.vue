<template>
  <!-- Approval Waiting -->
  <multi-purpose-modal v-if="approvalLoading" :header="$t('bountyPlatform.multiPurposeModal.approveAmount.header')"
  ...
  <!-- Approval Modal -->
  <div v-else v-on-clickaway="hideModal" @keydown.esc.exact="hideModal"
    ...
    <!-- Header -->
    <h5 class="text-center font-bold text-2xl text-c-primary">
    ...
    <label v-if="balance.primary.hasApproved" for="amountToApprove"
      class="text-c-text font-bold text-lg">{{ $t('bountyPlatform.sidebarContextual.amountToApprove') }}</label>
    <!-- Amount Input -->
    ...
    <!-- Approve Button -->
    <button v-if="balance.primary.hasApproved" :disabled="approvalLoading" @click="approveBalance"
      class="w-full max-w-xxs btn-primary bg-c-primary transform hover:scale-md focus:scale-md duration-200 ease-out transition-all origin-bottom-left text-c-light font-extrabold text-lg rounded-tl-2xl rounded-br-2xl rounded-tr-md rounded-bl-md px-6 py-1_5 mt-3">{{ $t('bountyPlatform.sidebarContextual.buttonApprove') }}</button>
    <p :class="approvalError && balance.primary.hasApproved ? 'text-c-danger' : 'text-transparent'" class="text-xs px-3 mt-3">
