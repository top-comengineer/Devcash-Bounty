<template>
  <nuxt-link :to="localePath({ name: 'bountyplatform-bounty-id', params: { id: bounty.id } })" :class="{
    ...
    <div class="w-full md:w-3/7 flex flex-col flex-wrap justify-center items-start px-4">
      <div class="flex flex-row flex-wrap items-center">
        ...
        <!-- Status Badge -->
        <div v-if="status == 'completed' || status == 'expired'"
          ...
          class="border text-sm font-bold px-3 py-0_5 rounded-full my-1_5">{{ isReclaimable ?
            $t("bountyPlatform.bountyCard.tag.reclaimable") : status == "expired"
              ? $t("bountyPlatform.bountyCard.tag.expired") : status == "completed" ? $t("bountyPlatform.bountyCard.tag.completed") : '' }}
        </div>
      </div>
    ...
    <div class="w-full md:w-2/7 flex flex-col justify-center order-last md:order-none items-start md:items-end px-4">
      <!-- Submissions Left -->
    ...
    <div class="w-full md:w-2/7 flex flex-col justify-center items-start md:items-end px-4">
      <h4 class="text-c-primary font-extrabold text-xl text-left md:text-right">
        {{ $store.state.devcashData.balancePrimary.symbol }}{{ $store.state.devcashData.ethIsPrimary ? ethAmount : amount }}</h4>
      <h5 class="text-lg text-left md:text-right mt-1">+
