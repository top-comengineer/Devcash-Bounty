<template>
  <div :class="type == 'sign' ? 'shadow-4xl' : 'shadow-2xl-sec'"
    ...
    <div v-if="!loggingInLoading" class="w-full max-w-xxs flex flex-col mt-4">
      <!-- MetaMask Button -->
      <button @click="signIn(walletProviders.metamask)"
        ...
        <div class="flex flex-row flex-1 justify-center">
          <h4 class="whitespace-no-wrap text-c-dark text-lg font-extrabold ml-8 mr-10">{{ true ? $t("wallet.metamask") :
            $t("wallet.getMetamask") }}</h4>
        </div>
      </button>
      <!-- Portis Button -->
      <button @click="signIn(walletProviders.portis)"
        ...
        <div class="flex flex-row flex-1 justify-center">
          <h4 class="whitespace-no-wrap text-c-dark text-lg font-extrabold ml-8 mr-10">{{ $t("wallet.portis") }}</h4>
        </div>
      </button>
      <!-- Authereum Button -->
      <button @click="signIn(walletProviders.authereum)"
        ...
        <div class="bg-c-background-ter h-12 w-12 p-2">
          <img class="w-full h-full" :src="require('~/assets/images/wallet-logos/authereum.svg')" alt="Authereum" />
        </div>
        <div class="flex flex-row flex-1 justify-center">
          <h4 class="whitespace-no-wrap text-c-dark text-lg font-extrabold ml-8 mr-10">{{ $t("wallet.authereum") }}</h4>
        </div>
      </button>
    </div>
  </div>
