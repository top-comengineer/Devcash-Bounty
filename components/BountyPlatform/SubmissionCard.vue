<template>
  <div class="flex flex-col transition-all duration-300 ease-out">
    <div
      ...
      <div class="bg-c-text-05 w-full flex flex-row flex-wrap justify-between items-center py-2">
        ...
        <div class="w-full md:w-auto flex flex-row md:items-center px-4 py-2">
          ...
          <h5 class="text-left ml-2 mr-3">
            <a :href="'https://etherscan.io/address/' + submission.creator" class="hover:underline" target="_blank">
              <span class="font-mono-jet font-bold">{{ submission.creator.substring(0, 6) + "..." +
                submission.creator.substring(submission.creator.length - 4) }}</span>
            </a>
            <span v-if="ubounty.title">-></span>
            <nuxt-link :to="localePath({ name: 'bountyplatform-bounty-id', params: { id: ubounty.id } })"
              v-if="ubounty.title && (page == 'bountyHunter' || page == 'bountyManager')"
              class="font-extrabold hover:underline">{{ ubounty.title }}</nuxt-link>
        ...
        <div class="w-full md:w-auto flex flex-row flex-wrap items-center px-4 md:pr-0">
          ...
          <a v-if="submission.status == 'pending' || (submission.blockHash && submission.status == 'approved')"
            :href="submission.status == 'pending' ? 'https://etherscan.io/address/' + ubounty.bountyChest : 'https://etherscan.io/tx/' + submission.blockHash"
            class="hover:underline flex flex-col my-2" target="_blank">
            <h5 :class="{
          ...
          <div class="flex flex-col md:mx-4 my-2">
            ...
            <div
              ...
              <button @click="approveClicked"
                ...
                <h5 class="text-c-success font-extrabold mx-1">{{ $t('bountyPlatform.bountyManager.approve') }}</h5>
              </button>
              <!-- Reject Button -->
              <button @click="rejectClicked"
                ...
                <h5 class="text-c-danger font-extrabold mx-1">{{ $t('bountyPlatform.bountyManager.reject') }}</h5>
              </button>
            </div>
      ...
      <div class="w-full flex flex-col flex-wrap px-4 md:px-6 py-6">
        ...
        <div class="flex flex-row justify-between items-end flex-wrap">
          ...
          <div class="bg-c-text-05 border border-c-text-10 rounded-lg flex flex-row flex-wrap items-start mt-8 mr-6">
            <div class="flex flex-col px-4 py-2 mr-4">
              <p class="text-sm opacity-75">{{ $t("bountyPlatform.singleBounty.submissions.submissionDate") }}</p>
            ...
            <div v-if="submission.contactName" class="flex flex-col px-4 py-2 mr-4">
              <p class="text-sm opacity-75">{{ $t("bountyPlatform.singleBounty.submissions.contactName") }}</p>
              <p class="font-bold mt-1">{{ submission.contactName }}</p>
            </div>
            <div v-if="submission.contactEmail" class="flex flex-col px-4 py-2 mr-4">
              <p class="text-sm opacity-75">{{ $t("bountyPlatform.singleBounty.submissions.contactEmail") }}</p>
              <p class="font-bold mt-1">{{ submission.contactEmail }}</p>
            </div>
          </div>
          <!-- Show Feedback -->
          <button v-if="submission.feedback" @click.prevent="isFeedbackVisible = !isFeedbackVisible"
    ...
    <transition name="feedbackTransition">
      <feedback-card v-if="isFeedbackVisible && submission.feedback" class="mt-4" :feedbackMessage="submission.feedback"
