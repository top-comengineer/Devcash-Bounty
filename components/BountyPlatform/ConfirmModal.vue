<template>
  <div
    ...
    <!-- Header -->
    <div :class="{ 'bg-c-danger': type == 'reject', 'bg-c-success': type == 'approve' }"
      class="w-full flex flex-row justify-center px-4 py-3">
      <h5 class="text-center text-c-background text-2xl font-bold">{{ type == 'reject' ?
        $t("bountyPlatform.confirmModal.confirmToReject.header") :
        $t("bountyPlatform.confirmModal.confirmToApprove.header") }}</h5>
    </div>
    <!-- Paragraph -->
    <p class="text-center px-3 md:px-8 mt-4 mb-2">{{ type == 'reject' ?
      $t("bountyPlatform.confirmModal.confirmToReject.paragraph") :
      $t("bountyPlatform.confirmModal.confirmToApprove.paragraph") }}</p>
    <!-- Amount & Address Part -->
    <div class="w-full flex flex-col items-center my-4 px-2">
      <div class="flex flex-col flex-wrap justify-center rounded-xl px-4 py-2 shadow-xl border border-c-text-10">
        <!-- DEV Amount -->
        <p :class="{ 'text-c-danger': type == 'reject', 'text-c-success': type == 'approve' }"
          class="text-xl font-bold text-center">{D}{{ amount }}</p>
        <!-- ETH & USD Amount -->
        <p class="text-center opacity-75 mt-1">{{ `(Ξ${ethAmount}` }}</p>
      </div>
      <!-- Indicator Icon -->
      <Icon class="my-3 w-8 h-8" :colorClass="type == 'reject' ? 'text-c-danger' : 'text-c-success'"
        :type="type == 'reject' ? 'cancel' : 'arrow-down'" />
      <!-- Avatar & Address -->
      <div class="flex flex-row flex-wrap justify-center rounded-xl px-4 py-2 shadow-xl border border-c-text-10">
    ...
    <div class="w-full flex flex-col bg-c-text-05 px-2 md:px-4 pt-6 pb-8 my-5">
      <p class="text-center text-xl font-bold px-3">
        ...
        <span class="text-normal font-normal text-sm opacity-75">{{
          $t("bountyPlatform.confirmModal.inputFeedback.optional")
        }}</span>
      </p>
      <client-only>
    ...
    <div class="w-full flex flex-row flex-wrap pt-2 pb-4 px-4">
      ...
      <div class="w-full md:w-1/2 flex flex-row p-2">
        <button @click.prevent="cancelCallback"
          class="w-full text-lg font-bold bg-c-background-sec text-c-text border-2 border-c-text btn-text-qua px-4 py-1_5 transform transition-all hover:scale-md focus:scale-md duration-300 ease-out origin-bottom-left rounded-tl-2xl rounded-br-2xl rounded-bl-md rounded-tr-md">{{
            type == 'reject' ? $t("bountyPlatform.confirmModal.confirmToReject.buttonCancel") :
            $t("bountyPlatform.confirmModal.confirmToApprove.buttonCancel") }}</button>
      </div>
      <!-- Confirm Button -->
      <div class="w-full md:w-1/2 flex flex-row p-2 order-first md:order-last">
        <button :disabled="loading" @click.prevent="confirmClicked"
          ...
          class="w-full text-lg font-bold text-c-background border-2 btn-text-qua px-4 py-1_5 transform transition-all hover:scale-md focus:scale-md duration-300 ease-out origin-bottom-left rounded-tl-2xl rounded-br-2xl rounded-bl-md rounded-tr-md">{{
            type == 'reject' ? $t("bountyPlatform.confirmModal.confirmToReject.buttonConfirm") :
            $t("bountyPlatform.confirmModal.confirmToApprove.buttonConfirm") }}</button>
      </div>
    </div>
  </div>
</template>
