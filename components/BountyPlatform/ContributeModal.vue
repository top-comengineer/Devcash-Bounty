<template>
  ...
  <div v-if="contributeLoading" class="w-full max-w-xl flex flex-row justify-center mx-auto">
    <multi-purpose-modal class="mx-auto" :header="$t('bountyPlatform.multiPurposeModal.contribute.header')"
      :paragraph="$t('bountyPlatform.multiPurposeModal.contribute.paragraph')"
      :imgSrc="require('~/assets/images/illustrations/foreground/devcash-d.svg')" :hasSpinner="true" />
  </div>
  <div v-else
    ...
    <div class="absolute top-0 right-0 z-40">
      <button @mouseover="isCloseHovered = true" @mouseleave="isCloseHovered = false" @focus="isCloseFocused = true"
        @blur="isCloseFocused = false" @click="closeModal()"
    ...
    <!-- Greeting Card -->
    <GreetingCard :header="$t('bountyPlatform.singleBounty.contribute.cardHeader')"
      ...
      type="contribute" class="my-1 md:my-2" />
    <!-- Card for Amount -->
    <div
      ...
      <div v-if="isLoggedIn" class="w-full flex flex-row flex-wrap">
        ...
        <div class="w-full flex flex-row flex-wrap justify-center my-2">
          ...
          <div class="w-full md:w-48 flex flex-col items-center my-2 mx-3">
            <button :disabled="contributeLoading"
              @click="() => preFillClicked($store.state.devcashData.ethIsPrimary ? 0.02 : 10000)"
              class="text-c-light btn-secondary w-full transform hover:scale-md focus:scale-md bg-c-secondary transition-all origin-bottom-left duration-200 ease-out font-extrabold text-xl rounded-tl-2xl rounded-br-2xl rounded-tr-md rounded-bl-md px-8 py-2 my-2 mx-2">{{ `${$store.state.devcashData.balancePrimary.symbol}${$store.state.devcashData.ethIsPrimary ? '0.02' : '10,000'}` }}</button>
          </div>
          <!-- {D}20,000 Button -->
          <div class="w-full md:w-48 flex flex-col items-center my-2 mx-3">
            <button :disabled="contributeLoading"
              @click="() => preFillClicked($store.state.devcashData.ethIsPrimary ? 0.04 : 20000)"
              class="text-c-light btn-secondary w-full transform hover:scale-md focus:scale-md bg-c-secondary transition-all origin-bottom-left duration-200 ease-out font-extrabold text-xl rounded-tl-2xl rounded-br-2xl rounded-tr-md rounded-bl-md px-8 py-2 my-2 mx-2">{{ `${$store.state.devcashData.balancePrimary.symbol}${$store.state.devcashData.ethIsPrimary ? '0.04' : '20,000'}` }}</button>
          </div>
          <!-- {D}40,000 Button -->
          <div class="w-full md:w-48 flex flex-col items-center my-2 mx-3">
            <button :disabled="contributeLoading"
              @click="() => preFillClicked($store.state.devcashData.ethIsPrimary ? 0.08 : 40000)"
              class="text-c-light btn-secondary w-full transform hover:scale-md focus:scale-md bg-c-secondary transition-all origin-bottom-left duration-200 ease-out font-extrabold text-xl rounded-tl-2xl rounded-br-2xl rounded-tr-md rounded-bl-md px-8 py-2 my-2 mx-2">{{ `${$store.state.devcashData.balancePrimary.symbol}${$store.state.devcashData.ethIsPrimary ? '0.08' : '40,000'}` }}</button>
          </div>
        </div>
      </div>
        ...
        <div class="w-full lg:w-2/3 flex flex-col mt-8 mb-2 mx-auto">
          <div class="w-full flex flex-row justify-center items-start flex-wrap">
            ...
            <div class="w-full md:flex-1 flex flex-row items-center relative">
              <!-- Amount Symbol -->
              <span
                class="absolute top-1/2 transform -translate-y-1/2 ml-2 text-lg font-bold">{{ $store.state.devcashData.ethIsPrimary ? `&nbsp;${$store.state.devcashData.balancePrimary.symbol}` : $store.state.devcashData.balancePrimary.symbol }}</span>
              <input id="customAmount"
                ...
                type="number" v-model="customAmount" :placeholder="
                  $t(
                    'bountyPlatform.singleBounty.contribute.customAmountPlaceholder'
                  )
                " @focus="amountError ? amountError = false : null" @blur="() => validateAmount(customAmount)" />
            </div>
            <!-- Contribute Button -->
            <button :disabled="contributeLoading" @click="contributeClicked"
      ...
      <!-- Divider -->
      <div v-if="isLoggedIn" class="bg-c-text w-full h-px2 opacity-10 rounded-tl-full rounded-br-full mb-6"></div>
