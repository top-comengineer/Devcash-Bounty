<template>
  <transition name="iboBarTransition">
    <div v-if="!$store.state.isIBOBarClosed && !isBarClosed"
      ...
      <!-- Join IBO Button -->
      <nuxt-link :to="localePath('bountyplatform')" @click.native="closeIBOBar"
        class="btn-text-sec bg-c-light text-c-secondary transform hover:scale-md focus:scale-md transition-all duration-200 ease-out origin-bottom-left max-w-full font-extrabold text-xl rounded-tl-2xl rounded-br-2xl rounded-tr-md rounded-bl-md px-12 py-2 mx-5 my-3">{{ $t('iboBar.buttonJoinIBO') }}</nuxt-link>
      ...
      <div class="flex flex-col mx-5 mt-4 mb-3">
        ...
        <div class="h-8 w-full max-w-56 md:w-56 bg-c-light-35 rounded-md overflow-hidden">
          <div :style="'width:' + (calculateDistributionRatio() * 100).toFixed(2) + '%'" class="h-full bg-c-light"></div>
        </div>
        <!-- Distributed Text -->
        <h4 class="font-bold mt-2 text-c-light">{{ $t('iboBar.devDistributed').replace('1%',
          (this.devDistributed).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 })) }}</h4>
      </div>
      <!-- Close Button -->
      <div class="absolute top-0 right-0 mt-2 mr-2">
        <button @mouseover="isCloseHovered = true" @mouseleave="isCloseHovered = false" @focus="isCloseFocused = true"
          @blur="isCloseFocused = false" @click="closeIBOBar">
          <Icon :class="isCloseHovered || isCloseFocused ? 'scale-120' : 'scale-100'" colorClass="text-c-light"
            class="w-8 h-8 transition-all duration-200 ease-out transform" type="cancel" />
        </button>
      </div>
    </div>
  </transition>
