<template>
  <div class="w-full flex flex-col justify-center items-center px-1 md:px-4">
    ...
    <transition name="modalBgTransition">
      <div v-if="isSubmissionModalOpen"
    ...
    <!-- Contribute Modal -->
    <transition name="modalBgTransition">
      <div v-if="isContributeModalOpen"
    ...
    <!-- Confirm Modal -->
    <transition name="modalBgTransition">
      <div v-if="isConfirmModalOpen" class="bg-c-background-75 w-full h-screen fixed flex flex-row left-0 top-0 modal">
        <div
          ...
          <ConfirmModal address="0x569DE003A10dB0E057f009AA516673C418e7Fdf1" :type="confirmModalType"
            :item="confirmModalItem" :cancelCallback="closeConfirmModal" :confirmCallback="confirmConfirmModal" />
        </div>
      </div>
    </transition>
    <!-- Header Card -->
    <div
      ...
      <div class="w-full md:w-1/2 flex flex-col justify-center items-start my-3">
        ...
        <div class="flex flex-row items-center mt-1">
          ...
          <a :href="'https://etherscan.io/address/' + bounty.bountyChest" class="hover:underline mr-1" target="_blank">
          ...
          <button @click.prevent="copyAddress(bounty.bountyChest)"
      ...
      <div class="w-full md:w-1/2 flex flex-col justify-center items-start md:items-end my-3">
        <a :href="'https://etherscan.io/address/' + bounty.bountyChest"
          class="hover:underline flex flex-col justify-center md:items-end" target="_blank">
          <!-- Bounty Amount in Devcash -->
          <h2 class="font-extrabold text-2xl md:text-3xl text-left md:text-right">
            {{ $store.state.devcashData.balancePrimary.symbol }}{{ $store.state.devcashData.ethIsPrimary ? ethAmount :
              amount }}
          </h2>
          <!-- Bounty Amount in ETH & USD -->
          <h3 class="text-lg md:text-xl text-left md:text-right mt-1">+
    ...
    <div class="w-full flex flex-row flex-wrap">
      ...
      <div
        ...
        <div :class="status == 'completed' ? 'bg-c-success' : status == 'expired' ? 'bg-c-pending' : 'bg-c-secondary'"
          ...
          <div v-if="status == 'active' && isLoggedIn" class="w-full flex flex-col items-center">
            ...
            <button @click="isContributeModalOpen = true"
          ...
          <div v-else-if="status == 'active' && !isLoggedIn" class="w-full flex flex-col items-center">
            <p class="text-c-light font-bold text-xl text-center">{{ $t("bountyPlatform.singleBounty.signInToHunt") }}</p>
            <button @click.prevent="toggleSignInModal"
            ...
            <!-- Sign In Modal -->
            <transition name="modalBgTransition">
              <div v-if="isSignInModalOpen && !isLoggedIn"
                class="bg-c-background-75 w-full h-screen fixed flex flex-row justify-center items-center left-0 top-0 modal">
                <div class="w-full md:max-w-xl h-full flex flex-row justify-center items-center px-2 pt-16 pb-12">
                  <sign-in-card v-on-clickaway="closeSignInModal" :closeModal="closeSignInModal" type="sign" />
                </div>
              </div>
            </transition>
          </div>
          ...
          <div v-else-if="status == 'expired'"
            ...
            <button v-if="isReclaimable" @click.prevent="reclaim"
            ...
            <!-- Reclaim Waiting -->
            <transition name="modalBgTransition">
              <div v-if="isReclaimable && reclaimConfirmWindowOpen"
                class="bg-c-background-75 w-full h-screen fixed flex flex-row justify-center items-center left-0 top-0 modal">
                <div class="max-w-xl h-full flex flex-row justify-center items-center px-2 pt-24 pb-12 md:pt-36">
                  <multi-purpose-modal :header="$t('bountyPlatform.multiPurposeModal.reclaim.header')"
        ...
        <div class="w-full flex flex-col px-6 py-6">
          ...
          <div class="text-left text-sm mt-2">
            ...
            <span v-else-if="status == 'expired'" class="font-bold">{{ $t('bountyPlatform.bountyCard.tag.expired')
            }}</span>
        ...
        <div class="w-full flex flex-col px-6">
          <!-- Creator Card -->
          <CreatorCard :name="bounty.contactName" :email="bounty.contactEmail" :address="bounty.creator" />
        </div>
        ...
        <div class="w-full flex flex-row px-6 mt-6 pb-6">
          <h5>
            ...
            <span class="border border-c-text-10 shadow-lg font-bold opacity-100 px-3 py-1 rounded-full">{{
      ...
      <div
        ...
        <div
          ...
          <div class="w-full flex flex-row relative">
            ...
            <button :class="[activeTab == 'submissions' ? 'text-c-light' : 'hover:bg-c-text-15 focus:bg-c-text-15']"
            ...
            <button :class="[activeTab == 'comments' ? 'text-c-light' : 'hover:bg-c-text-15 focus:bg-c-text-15']"
            ...
            <button :class="[activeTab == 'activity' ? 'text-c-light' : 'hover:bg-c-text-15 focus:bg-c-text-15']"
        ...
        <div v-if="activeTab == 'submissions'" class="w-full">
          <div v-if="submissions.length > 0" class="w-full flex flex-col">
            ...
            <SubmissionCard class="my-2" :perspective="bounty.creator == loggedInAccount ? 'manager' : 'hunter'"
              v-for="(item, i) in submissions" :key="i" :submission="item" :ubounty="bounty"
            ...
            <div v-if="hasMoreSubmissions && !submissionsLoading" class="flex flex-row justify-center mt-2">
              <button @click="loadMoreSubmissions()"
        ...
        <div v-if="activeTab == 'activity'" class="w-full flex flex-col">
          <ActivityCard v-if="status == 'completed' && bounty.activity.length > 0" class="my-2" perspective="general"
            messageType="bountyCompleted" :bountyName="bounty.title"
            :date="formatDate(bounty.activity[bounty.activity.length - 1]).createdAt" />
          <ActivityCard class="my-2" perspective="general" v-for="(item, i) in bounty.activity" :item="item" :key="i"
            :messageType="item.type ? item.type : 'submissionMade'" :address="item.creator"
            :date="formatDate(item.createdAt)" />
          <ActivityCard class="my-2" perspective="general" messageType="bountyCreated"
