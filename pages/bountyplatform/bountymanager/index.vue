    ...
    <div v-if="isLoggedIn" class="w-full flex flex-row flex-wrap justify-center">
      ...
      <div
        ...
        <div class="flex flex-row flex-wrap justify-between items-center">
          <!-- Header -->
          <h2 class="text-2xl font-extrabold mx-4 my-2">{{ $t('bountyPlatform.bountyManager.submissionsReceived') }}</h2>
          <!-- Checkboxes -->
        ...
        <div v-else-if="submissions.length > 0" class="w-full flex flex-col flex-wrap my-4">
          <SubmissionCard class="my-2" perspective="manager" v-for="(item, i) in filteredSubmissions" :key="i"
            :submission="item" :ubounty="item.ubounty" :approveClicked="() => showConfirmModal(item, 'approve')"
            :rejectClicked="() => showConfirmModal(item, 'reject')" page="bountyManager" />
        </div>
        ...
        <div v-else class="w-full flex flex-col justify-center items-center flex-wrap">
          ...
          <h6 class="opacity-75 text-center mt-4 md:max-w-xs">{{ $t('bountyPlatform.bountyManager.noSubmissionsReceived')
          }}
          </h6>
      ...
      <div
        ...
        <div class="flex flex-row flex-wrap justify-between items-center">
          <!-- Header -->
          <h2 class="text-2xl font-extrabold mx-4 my-2">{{ $t('bountyPlatform.bountyManager.yourBounties') }}</h2>
          ...
          <div v-if="bounties.length > 0 && !bountiesLoading" class="flex flex-row flex-wrap items-center">
            <div class="flex flex-row flex-wrap my-2 mr-10">
              ...
              <div class="flex flex-col items-center mx-4">
                ...
                <h5 class="font-bold mt-1">{{ $store.state.devcashData.balancePrimary.symbol }}{{ totalBountyAmountDisplay
                }} + {{ $store.state.devcashData.balanceSecondary.symbol }}{{ totalBountyAmountSecondaryDisplay }}</h5>
              </div>
            </div>
        ...
        <div v-else-if="bounties.length > 0" class="w-full flex flex-col flex-wrap my-4">
          <BountyCard type="secondary" v-for="(item, i) in filteredBounties" :key="i" class="my-2" perspective="manager"
            :bounty="item" />
        </div>
        <!-- If there are no bounties -->
        <div v-else class="w-full flex flex-col justify-center items-center flex-wrap">
          <img class="max-w-xxxs h-auto relative" :src="require('~/assets/images/illustrations/foreground/bounty.svg')"
            alt="No Bounty Illustration" />
          <h6 class="opacity-75 text-center mt-4 md:max-w-xs">{{ $t('bountyPlatform.bountyManager.noYourBounties') }}</h6>
          <!-- Post Bounties Call to Action -->
          <div class="flex flex-row justify-center">
            <nuxt-link :to="localePath('bountyplatform-post')"
              class="btn-primary bg-c-primary text-c-light transform hover:scale-md focus:scale-md transition-all ease-out duration-200 origin-bottom-left font-extrabold text-xl rounded-tl-2xl rounded-br-2xl rounded-tr-md rounded-bl-md px-12 py-2 my-6">{{
